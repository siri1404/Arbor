 ...
 * 2. MPSC Queue with flat combining for reduced contention
 * 3. MPMC Queue with elimination array and helping mechanism
 ...
 * - Adaptive backoff with exponential + jitter (AdaptiveBackoff class)
 * - Batch coalescing for improved throughput (push_batch/pop_batch methods)
 * - NUMA-aware memory allocation via libnuma (NUMAAllocator class)
 * - Formal memory ordering analysis documented per operation
 * - Hardware prefetch hints for sequential access patterns
 * - Elimination arrays for matched producer/consumer pairs
...
// NUMA support via libnuma on Linux
#if defined(__linux__) && __has_include(<numa.h>)
#include <numa.h>
#include <numaif.h>
#define ARBOR_HAS_NUMA 1
#else
#define ARBOR_HAS_NUMA 0
#endif
 ...
 * 
 * Usage:
 *   NUMAAllocator allocator(0);  // NUMA node 0
...
public:
...
#if ARBOR_HAS_NUMA
        if (numa_available_) {
            // Allocate on specific NUMA node with alignment
            void* ptr = numa_alloc_onnode(size + alignment, node_);
    ...
    // Get memory statistics for NUMA node
    struct NUMAStats {
        int node;
        size_t total_bytes;
        size_t free_bytes;
    ...
    NUMAStats get_stats() const {
        NUMAStats stats{node_, 0, 0, numa_available_};
#if ARBOR_HAS_NUMA
        if (numa_available_) {
            long long free_size;
            stats.total_bytes = numa_node_size64(node_, &free_size);
            stats.free_bytes = static_cast<size_t>(free_size);
        }
#endif
        return stats;
    }
    ...
    bool available() const { return numa_available_; }
    
    // Bind current thread to NUMA node for memory affinity
    bool bind_thread() {
#if ARBOR_HAS_NUMA
        if (numa_available_) {
            struct bitmask* nodemask = numa_allocate_nodemask();
            numa_bitmask_setbit(nodemask, node_);
            numa_bind(nodemask);
 ...
 * 
 * Implemented optimizations beyond basic Lamport:
 * 1. Batch push/pop (push_batch, pop_batch) - amortizes memory barriers
 * 2. Hardware prefetch hints (_mm_prefetch for sequential access pattern)
 * 3. Cached indices (cached_tail_, cached_head_) - reduces cross-core traffic
 *    Producer caches tail to avoid reading consumer's atomic
 *    Consumer caches head to avoid reading producer's atomic
 * 4. NUMA-aware allocation via NUMAAllocator class (see above)
 *    Use: Allocate queue on same NUMA node as producer/consumer threads
 * 
 * Cache line layout (prevents false sharing):
 * - Line 0: head_ + cached_tail_ (producer-only)
 * - Line 1: tail_ + cached_head_ (consumer-only)
 * - Line 2+: buffer_ (shared, accessed sequentially)
 * 
 * Guarantees:
...
public:
     ...
     * 
     * Memory ordering analysis:
     * - head_ load relaxed: Only producer writes, no ordering needed
     * - tail_ load acquire: Synchronizes with consumer's release store
     * - head_ store release: Publishes data written to buffer_[head]
     * 
     * The release-acquire pair ensures the consumer sees the written data.
     */
    template<typename U>
    ...
    bool push(U&& item) noexcept {
        ...
        // Check cached tail first (avoid cross-core read) - common case: not full
        if (next == cached_tail_) [[unlikely]] {
            // Cache miss - read actual tail with acquire semantics
        ...
        // Release store ensures buffer write is visible before head increment
     ...
     * 
     * Memory ordering analysis:
     * - tail_ load relaxed: Only consumer writes, no ordering needed
     * - head_ load acquire: Synchronizes with producer's release store
     * - tail_ store release: Publishes slot as available for producer
     */
    __attribute__((always_inline))
    std::optional<T> pop() noexcept {
        const size_t tail = tail_.load(std::memory_order_relaxed);
        
        // Check cached head first - common case: not empty
        ...
        // Release store ensures we've finished reading before allowing overwrite
...
//=============================================================================
// MPSC QUEUE - Flat Combining for Reduced Contention
//=============================================================================
 ...
 * 
 * Implemented optimizations:
 * 1. Sequence-based ABA prevention (slot.sequence counter)
 * 2. Flat combining under high contention (try_combine_push method)
 *    - After MAX_BACKOFF_SPINS/2 spins, thread attempts to become combiner
 *    - Combiner acquires lock and performs operation with reduced contention
 *    - Fallback from lock-free to mutex-protected under pathological cases
 * 3. Wait-free consumer path (single consumer, no CAS needed)
 * 4. Batch drain via pop_batch for amortized overhead
 * 
 * Contention behavior:
 * - Low contention: Direct CAS (zero lock overhead)
 * - High contention: Flat combining (serializes writes, reduces cache bouncing)
 * 
 * Memory ordering:
 * - head_: relaxed CAS with release on success (publishes slot claim)
 * - slot.sequence: acquire on read (sees data), release on write (publishes data)
 */
template<typename T, size_t Capacity>
class MPSCQueue {
    static_assert((Capacity & (Capacity - 1)) == 0, "Capacity must be power of 2");
...
//=============================================================================
// MPMC QUEUE - Elimination Array for Matched Pairs
//=============================================================================

/**
 * MPMC Queue with Elimination Optimization
 * 
 * Implemented optimizations:
 * 1. Elimination array (ELIM_SIZE slots for direct producer-consumer matching)
 *    - Push: Publishes pointer in elimination slot, waits briefly for consumer
 *    - Pop: Checks elimination slot first, transfers directly without queue access
 *    - Result: O(1) latency under balanced load (no queue contention)
 * 2. Sequence-based ABA prevention (slot.sequence ensures correct ordering)
 * 3. Adaptive backoff with timeout (gives up after MAX_BACKOFF_SPINS)
 * 
 * Guarantees:
 * - Lock-free (CAS-based, no blocking)
 * - Linearizable (elimination preserves order within matched pairs)
 * - No ABA problem (sequence counter increments monotonically)
 * 
 * Memory ordering:
 * - elimination_[]: acquire/release for publishing and consuming data pointers
 * - slot.sequence: acquire on read, release on write (same as MPSC)
 */
template<typename T, size_t Capacity>
class MPMCQueue {
    static_assert((Capacity & (Capacity - 1)) == 0, "Capacity must be power of 2");
